...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    let tickers = fs.existsSync(TICKERS_FILE)
      ? JSON.parse(fs.readFileSync(TICKERS_FILE, 'utf-8'))
      : [];
    tickers = tickers.filter((t: string) => t !== ticker);
    fs.writeFileSync(TICKERS_FILE, JSON.stringify(tickers, null, 2));
    return res.status(200).json({ message: 'Тикер удален' });
  } catch (error) {
    return res.status(500).json({ error: 'Ошибка при удалении тикера' });
  }
}